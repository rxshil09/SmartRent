...
class NotificationsService {
  ...
  /**
   * Sends a rental cancellation email.
   * @param {string} userEmail - The user's email.
   * @param {object} rentalDetails - Details of the rental.
   * @returns {Promise<boolean>}
   */
  async sendRentalCancellation(userEmail, rentalDetails) {
    const subject = 'Rental Cancellation - SmartRent';
    const html = `
      <div style="font-family: Arial, sans-serif; color: #333;">
        <h2>Rental Order Cancelled</h2>
        <p>Dear Customer,</p>
        <p>Your rental order has been cancelled.</p>
        <ul>
          <li>Order ID: ${rentalDetails.orderId}</li>
          <li>Product: ${rentalDetails.productName}</li>
        </ul>
        <p>If you did not request this cancellation or have any concerns, please get in touch with our team.</p>
        <p>Thank you,</p>
        <p>The SmartRent Team</p>
      </div>
    `;
    return this.sendEmail(userEmail, subject, html);
  }
